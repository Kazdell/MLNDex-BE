...
public class ChapterPagesController : ControllerBase
{
    ...
    public ChapterPagesController(IChapterPageService service) => _service = service;
    ...
    )
    {
        ...
        foreach (var file in pages)
        {
            ...
            if (file.Length > MaxFileSizeBytes)
                return BadRequest(new { message = $"File '{file.FileName}' vượt quá 20MB." });
        }
    ...
    public async Task<IActionResult> DeletePage(int pageId, CancellationToken cancellationToken)
    {
