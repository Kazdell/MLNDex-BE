    ...
    public class Program
    {
        public static void Main(string[] args)
        {
            ...
            builder.Services.AddScoped<
                Application.Interfaces.ReportSystem.IPlagiarismReportService,
                Application.Services.ReportSystem.PlagiarismReportService
            >();
            builder.Services.AddScoped<
                Application.Interfaces.ReportSystem.ITrustScoreService,
                Application.Services.ReportSystem.TrustScoreService
            >();
            ...
            if (ocrAvailable)
            {
                builder.Services.Configure<Application.Models.OCR.OcrSettings>(
                ...
                builder.Services.AddKeyedScoped<ITextRecognitionService>(
                    "tesseract",
                    (sp, key) => sp.GetRequiredService<TesseractOCRService>()
                );
                builder.Services.AddKeyedSingleton<ITextRecognitionService, PaddleOcrService>(
                    "paddle"
                );
            ...
            else
            {
                ...
                Console.WriteLine(
                    "[OCR] Native DLLs NOT found — OCR disabled (Azure Free Tier mode)."
                );
            }
            ...
            //enable jwt token
            var _authkey =
                builder.Configuration.GetValue<string>("JwtSettings:securitykey")
                ?? throw new Application.Exceptions.AppException(
                    Application.DTOs.Common.ErrorCodes.OPERATION_NOT_ALLOWED,
                    "JwtSettings:securitykey is missing from configuration."
                );
            builder
