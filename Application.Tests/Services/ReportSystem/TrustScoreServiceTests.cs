...
namespace Application.Tests.Services.ReportSystem
{
    public class TrustScoreServiceTests
    {
        ...
        public async Task RestoreTrustScore_ShouldUnlockTeam()
        {
            ...
            {
                TeamId = 1, TeamName = "Locked Team", Slug = "locked-team",
                TrustScore = 0, LockStatus = TeamLockStatus.LOCKED
            });
            await db.SaveChangesAsync();
        ...
        public async Task ApplyPenalty_ShouldSetCannotUpload_WhenScoreDropsToZero()
        {
            ...
            {
                ReportId = 1, ReporterId = 1,
                ContentType = ReportTargetType.User, ContentId = 2,
                Status = ReportStatus.Pending, Reason = ReportReason.Plagiarism
            });
            await db.SaveChangesAsync();
