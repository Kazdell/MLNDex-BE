...
namespace Application.Services.Auth
{
    public class AuthService : IAuthService
    {
        ...
        public async Task<ServiceResult> RegisterAsync(RegisterDto dto)
        {
            ...
            var existingEmail = await _context.Users.AnyAsync(u => u.Email == dto.Email.ToLower());
            if (existingEmail)
        ...
        public async Task<AuthResponseDto?> LoginAsync(LoginDto dto)
        {
            ...
            if (!string.IsNullOrEmpty(adminUsername) && !string.IsNullOrEmpty(adminPassword))
            {
                isAdminLogin =
                    (
                        dto.Username.Equals(adminUsername, StringComparison.OrdinalIgnoreCase)
                        || (
                            !string.IsNullOrEmpty(adminEmail)
                            && dto.Username.Equals(adminEmail, StringComparison.OrdinalIgnoreCase)
                        )
                    )
            ...
            var user = await _context
                ...
                .FirstOrDefaultAsync(u =>
                    u.Username == dto.Username.ToLower() || u.Email == dto.Username.ToLower()
                );
            ...
            if (isAdminLogin)
            {
                ...
                if (!user.UserRoles.Any(ur => ur.RoleId == adminRole.RoleId))
                {
        ...
        public async Task<AuthResponseDto?> GoogleLoginAsync(GoogleLoginDto dto)
        {
            ...
            var user = await FindOrMergeAccountAsync(
                socialUser.Email,
                socialUser.SocialId,
                null,
                socialUser.Name
        ...
        {
            // Gán role READER
            var readerRole = await _context.Roles.FirstOrDefaultAsync(r =>
                r.RoleName == RoleName.READER
            );
            if (readerRole != null)
            {
                user.UserRoles.Add(
                    new UserRole { RoleId = readerRole.RoleId, AssignedAt = DateTime.UtcNow }
                );
            }
        ...
        private bool IsReservedUsernameOrEmail(string username, string email)
        {
            ...
            if (!string.IsNullOrEmpty(adminUsername) && normalizedUsername == adminUsername)
                return true;
            if (!string.IsNullOrEmpty(adminEmail) && normalizedEmail == adminEmail)
                return true;

            if (!string.IsNullOrEmpty(adminEmail) && normalizedUsername == adminEmail)
                return true;
            if (!string.IsNullOrEmpty(adminUsername) && normalizedEmail == adminUsername)
                return true;
