    ...
    public class ModerationService : IModerationService
    {
        ...
        public async Task<AiModerationResultDto> RunAiModerationAsync(int chapterId)
        {
            ...
            else
            {
                ...
                // ── +5 uy tín cho creator ──────────────────────────────
                var creatorProfile = await _db.CreatorProfiles
                    .FirstOrDefaultAsync(cp => cp.CreatorId == chapter.Series.CreatorId);
                if (creatorProfile != null)
                {
                    creatorProfile.ReputationScore = Math.Min(100, creatorProfile.ReputationScore + 5);
        ...
        public async Task<AiModerationResultDto> RunTranslationModerationAsync(int translationId)
        {
            ...
            else
            {
                ...
                // ── +5 uy tín cho translation team ────────────────────
                var team = await _db.TranslationTeams
                    .FirstOrDefaultAsync(t => t.TeamId == translation.TeamId);
                if (team != null)
                {
                    team.ReputationScore = Math.Min(100, team.ReputationScore + 5);
                    _logger.LogInformation(
