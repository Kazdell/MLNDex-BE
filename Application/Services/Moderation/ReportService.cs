...
namespace Application.Services.Moderation
{
    public class ReportService : IReportService
    {
        ...
        {
            var baseQuery = _context.ModerationQueues.Include(q => q.Reports).AsQueryable();
                ...
                .Select(q => new ModerationQueueDto
                {
                    ...
                    ContentTitle =
                        q.ContentType == ModerationQueueContentType.SERIES
                            ? _context
                                .Series.Where(s => s.SeriesId == q.ContentId)
                                .Select(s => s.Title)
                                .FirstOrDefault()
                            : _context
                                .Chapters.Where(c => c.ChapterId == q.ContentId)
                                .Select(c => c.Title)
                                .FirstOrDefault(),
                    AuthorName =
                        q.ContentType == ModerationQueueContentType.SERIES
                            ? _context
                                .Series.Where(s => s.SeriesId == q.ContentId)
                                .Select(s => s.Creator.PenName)
                                .FirstOrDefault()
                            : _context
                                .Chapters.Where(c => c.ChapterId == q.ContentId)
                                .Select(c => c.Series.Creator.PenName)
                                .FirstOrDefault(),
                    Reports = q
        ...
        )
        {
            var queue =
                await _context
                    .ModerationQueues.Include(q => q.Reports)
            ...
            if (
                ...
                || queue.Status == QueueStatus.DISMISSED
