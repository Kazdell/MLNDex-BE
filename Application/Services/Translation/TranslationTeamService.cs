...
namespace Application.Services.Translation
{
    public class TranslationTeamService : ITranslationTeamService
    {
        ...
        public async Task<TranslationTeamDto> UpdateTeamAsync(int teamId, UpdateTranslationTeamDto updateDto)
        {
            ...
            team.UnlockEnabled = updateDto.UnlockEnabled;

            if (!updateDto.UnlockEnabled)
            {
                team.DefaultUnlockPriceCoins = null;
                team.DefaultFreeAfterDays = null;
                team.FreeAfterEnabled = false;
            }
            else
            {
                if (updateDto.DefaultUnlockPriceCoins.HasValue)
                    team.DefaultUnlockPriceCoins = updateDto.DefaultUnlockPriceCoins.Value;

                team.FreeAfterEnabled = updateDto.FreeAfterEnabled;

                if (!updateDto.FreeAfterEnabled)
                    team.DefaultFreeAfterDays = null;
                else if (updateDto.DefaultFreeAfterDays.HasValue)
                    team.DefaultFreeAfterDays = updateDto.DefaultFreeAfterDays.Value;
            }
